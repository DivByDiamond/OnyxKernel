//! SHA-256 (FIPS 180-4). Streaming API: `sha256_init` / `sha256_update` /
//! `sha256_final`, plus the one-shot `sha256` convenience wrapper.
//!
//! Correctness is pinned by known-answer tests (RFC 6234 vectors and block
//! boundary cases) living in `sha256_tests.rs`; they run on the host via
//! `cargo test -p onyx_core`.
...
// FIPS 180-4 names these functions Σ0/Σ1; keep the historical spelling.
