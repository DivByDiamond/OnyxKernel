...
- [x] **SMP-краш под `-smp 2` (idle-харт, "illegal instruction" / битые
      ...
      **Остаётся открытым**: после всех пяти фиксов выше
      воспроизводимость исходного краша(-ей) резко упала — ни одного
      бесконечного мусорного лупа за 6 прогонов подряд, — но изредка
      (не зациклено, чисто перехватывается и паникует с диагностикой)
      idle-харт всё ещё ловит `scause=1 (instruction access fault)
      sepc=0x0` — прыжок на нулевой адрес. Это уже НЕ бесконечный спам
      и не мусорные поля (kdump печатает валидный `hartid`), так что
      система деградирует чисто вместо каскадного краша, но откуда
      берётся `sepc=0` — ещё не найдено. Нужен отдельный аудит: либо
      ещё один путь, резюмирующий несохранённый/устаревший trap frame
      (по аналогии с п.2, но не через `G_HART_IDLE_TF`), либо гонка в
      `steal()`/`dequeue()` над `tf.sepc` конкретного процесса.
