...
- [ ] **A3. НЕ закрыто, sub-приоритет** — rv32-ветка (`arm_timer` пишет
      CLINT MMIO напрямую) не тронута, тот же класс проблемы там
      теоретически возможен, но rv32 — вторичный таргет проекта (см. также
      аналогичный открытый пункт rv32 у `usleep`-фикса от 2026-09-02 чуть
      выше). Нужен свой `mtrap_32.rs`, если/когда понадобится.
