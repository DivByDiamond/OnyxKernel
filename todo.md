...
- [x] **SMP-краш под `-smp 2` (idle-харт, "illegal instruction" / битые
      ...
      **Рекомендация для следующего захода**: пересобрать
      `onyx-kernel.elf` БЕЗ `--release` (полные debug-символы, без
      inlining), повторить ту же GDB-сессию (`qemu -s -S` +
      `target remote localhost:1234`), поставить breakpoint на вход в
      `srv::trap::handle` с условием на конкретный `scause`, и
      сравнить `tp`/`gp` на обоих хартах ДО и ПОСЛЕ фолта — если они
      ДЕЙСТВИТЕЛЬНО обнуляются, это укажет на реальную, ещё не найденную
      гонку поверх `tp`/`gp` (несмотря на `+reserve-x4`, который защищает
      только от компиляторных перевыделений, но не от рантайм-порчи
      данных, например записи через сырой указатель не в те байты).
