...
// TODO(dead-code): Cbw — used by unwired mass/transfer.rs (future mass-storage path, unwired — wire probe→transfer when xhci SCSI lands), keep. 2026-08-24
#[allow(dead_code)]
#[repr(C, packed)]
...
// TODO(dead-code): Csw — used by unwired mass/transfer.rs (future mass-storage path, unwired — wire probe→transfer when xhci SCSI lands), keep. 2026-08-24
#[allow(dead_code)]
#[repr(C, packed)]
...
// TODO(dead-code): CBW_SIG — used by unwired mass/transfer.rs (future mass-storage path, unwired — wire probe→transfer when xhci SCSI lands), keep. 2026-08-24
#[allow(dead_code)]
pub(super) const CBW_SIG: u32 = 0x43425355;
// TODO(dead-code): CBW_SIZE — used by unwired mass/transfer.rs (future mass-storage path, unwired — wire probe→transfer when xhci SCSI lands), keep. 2026-08-24
#[allow(dead_code)]
                ...
                if if_class == 0x08 && if_sub == 0x50 && if_proto == 0x50 {
                    ...
                    while j + 1 < config_data.len() && j < i + 80 {
                        ...
                        if etype == 5 && elen >= 7 {
                            ...
                            if ep_attr & 0x03 == 2 {
                                let _mps =
                                    (config_data[j + 5] as u16) << 8 | config_data[j + 4] as u16;
                                if ep_addr & 0x80 != 0 {
                                    bul_in = ep_addr;
                                } else {
                                    bul_out = ep_addr;
