...
use super::super::{
    G_ASYNCLIST_ENABLED, MAX_QTD, OP_USBSTS, QH_DEV_ADDR_SHIFT, QH_DTC, QH_EPS_HIGH, QH_MPL_SHIFT,
...
pub unsafe fn ehci_bulk_transfer(
    ...
    unsafe {
        ...
        (*qh).overlay_token = data_pid
            | QTD_CERR_3
            | (data_len.min(QTD_BUF_SIZE) << QTD_TOTAL_LEN_SHIFT)
            | QTD_ACTIVE;
        ...
        loop {
            ...
            for (i, &qtd_idx) in qtd_indices.iter().enumerate().take(data_qtds as usize) {
                let dt = qtd_ptr(qtd_idx);
