...
//! Host test builds compile the cfg(test) stubs, so the asm import is only
        ...
        pub unsafe fn $name() -> u64 {
            #[cfg(all(not(test), target_pointer_width = "64"))]
            {
                ...
                unsafe {
                    use core::arch::asm;
                    let v: u64;
                    asm!(concat!("csrr {0}, ", $csr), out(reg) v, options(nomem, nostack));
                    v
                }
            }
...
csr_read_u64!(read_sip, "sip");
csr_set_u64!(set_sip, "sip");
csr_clear_u64!(clear_sip, "sip");
