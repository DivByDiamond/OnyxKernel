...
pub unsafe fn sys_chown(path: u64, uid: u32, gid: u32) -> i64 { unsafe {
...
pub unsafe fn sys_fchown(fd: u64, uid: u32, gid: u32) -> i64 { unsafe {
...
pub unsafe fn sys_chmod(path: u64, mode: u64) -> i64 { unsafe {
    let mut path_buf = [0u8; 256];
    let path_len = match parse_user_path(path, &mut path_buf) {
        Some(l) => l,
        None => return Errno::Inval.as_i64(),
...
pub unsafe fn sys_fchmod(fd: u64, mode: u64) -> i64 { unsafe {
