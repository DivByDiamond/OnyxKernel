...
use onyx_core::formats::{onyxfs_growth_limit, onyxfs_growth_target, OnyfsSuper, ONYFS_BLOCK_SIZE};

pub unsafe fn mount(dev: usize, lba_offset: u32) -> KResult<()> { unsafe {
    crate::kinf!(
        "onyxfs",
        "mount dev=%d lba=%d",
        Arg::from(dev as u32),
...
unsafe fn blk_capacity_sectors(dev_idx: usize) -> u64 { unsafe {
...
unsafe fn grow_to_device() -> KResult<()> { unsafe {
...
pub(super) unsafe fn persist_superblock() -> KResult<()> { unsafe {
...
#[inline]
pub(super) unsafe fn inode_table_block_count() -> u32 { unsafe {
