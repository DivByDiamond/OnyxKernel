...
pub unsafe fn write_inode(ino: u32, inode: &OnyfsInode) -> KResult<()> { unsafe {
...
pub unsafe fn update_mtime(ino: u32) -> KResult<()> { unsafe {
...
pub unsafe fn set_mode(ino: u32, mode: u32) -> KResult<()> { unsafe {
    if G_VERSION == ONYFS_V1 {
        return Err(Errno::NoSys);
    }
    let mut inode = OnyfsInode {
...
pub unsafe fn set_uid_gid(ino: u32, uid: u32, gid: u32) -> KResult<()> { unsafe {
    if G_VERSION == ONYFS_V1 {
        return Err(Errno::NoSys);
    }
    let mut inode = OnyfsInode {
...
pub unsafe fn set_timestamps(ino: u32, mtime: u64, atime: u64) -> KResult<()> { unsafe {
    if G_VERSION == ONYFS_V1 {
        return Err(Errno::NoSys);
    }
    let mut inode = OnyfsInode {
