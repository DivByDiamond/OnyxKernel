use crate::fs::vfs::{
    FdToken, Fs, PERM_READ, PERM_WRITE, alloc_fd, fd_check, fd_clear, fd_get, fd_set, fd_token,
    resolve_path, with_mount,
};
use crate::fs::{devfs, fat32, ipcfs, onyxfs, procfs};
use onyx_core::errno::{Errno, KResult};

/// # Safety
///
/// Caller contract: path comes from the syscall layer's parse_user_path
/// (NUL-terminated, copied into kernel memory) or a kernel-side literal;
/// runs in the fd-owning context (kernel boot or the current process's
/// syscall on this hart).
pub unsafe fn open(path: &[u8], perms: u32) -> KResult<FdToken> {
    // SAFETY: the per-process fd slot is claimed via alloc_fd in this
    // context; G_ROOT_FS is read-only after boot-time mounting; all fd-table
    // writes use the checked idx from alloc_fd (see ops.rs # Safety).
    // resolve_path/with_mount uphold the FS_LOCK mount-context contract.
    unsafe {
        let (fs, subpath, slot) = resolve_path(path)?;
        let idx = alloc_fd(perms)?;
        // Bug (fs SERIOUS #3): if the lookup below fails, we must release the
        // FD slot we just allocated — otherwise every failed open() permanently
        // leaks an FD slot, and a process that retries open() in a loop will
        // exhaust its FD table (VFS_MAX_FDS = 16). We use a closure-style
        // pattern: do the lookup, and on Err, call fd_clear(idx) before
        // propagating the error.
        let (ino, size) = match fs {
            Fs::Proc => {
                let ino = match procfs::lookup(subpath) {
                    Ok(i) => i,
                    Err(e) => {
                        fd_clear(idx);
                        return Err(e);
                    }
                };
                let st = procfs::stat(ino)?;
                (ino, st.size)
            }
            Fs::Ipc => {
                let ino = match ipcfs::lookup(subpath) {
                    Ok(i) => i,
                    Err(e) => {
                        fd_clear(idx);
                        return Err(e);
                    }
                };
                let st = ipcfs::stat(ino)?;
                (ino, st.size)
            }
            Fs::Devfs => {
                let ino = match devfs::lookup(subpath) {
                    Ok(i) => i,
                    Err(e) => {
                        fd_clear(idx);
                        return Err(e);
                    }
                };
                let st = devfs::stat(ino)?;
                // /dev/ptmx is a clone node: each open claims a fresh pair
                // and the fd is rebound to that pair's master inode.
                if ino == devfs::DEVFS_PTMX_INO {
                    let pair = match crate::fs::pty::alloc() {
                        Ok(p) => p,
                        Err(e) => {
                            fd_clear(idx);
                            return Err(e);
                        }
                    };
                    (devfs::ptym_ino(pair), u32::MAX)
                } else {
                    (ino, st.size)
                }
            }
            Fs::Onyx => {
                // Lookup + permission check in the target mount's context:
                // the root fs keeps the singleton state, a secondary mount
                // is swapped in by with_mount for the duration.
                let r = with_mount(slot, || -> KResult<(u32, u32)> {
                    let mut st = onyxfs::OnyfsStat::default();
                    onyxfs::lookup(subpath, &mut st)?;
                    if !crate::fs::vfs::is_kernel_boot() {
                        let cur = crate::proc::current();
                        let is_root = cur.uid == 0
                            || crate::proc::current_ring() <= crate::proc::PROC_RING_ROOT;
                        if !is_root {
                            let owner_ok = cur.uid == st.uid;
                            let group_ok = cur.gid == st.gid;
                            let perm_bits = if owner_ok {
                                st.mode & 0o700
                            } else if group_ok {
                                (st.mode >> 3) & 0o700
                            } else {
                                st.mode & 0o007
                            };
                            if (perms & PERM_READ) != 0 && (perm_bits & 0o400) == 0 {
                                return Err(Errno::Perm);
                            }
                            if (perms & PERM_WRITE) != 0 && (perm_bits & 0o200) == 0 {
                                return Err(Errno::Perm);
                            }
                        }
                    }
                    Ok((st.ino, st.size.min(u32::MAX as u64) as u32))
                });
                match r {
                    Ok(v) => v,
                    Err(e) => {
                        fd_clear(idx);
                        return Err(e);
                    }
                }
            }
            Fs::Fat32 => {
                let r = with_mount(slot, || -> KResult<(u32, u32)> {
                    let mut cluster = 0u32;
                    let mut sz = 0u32;
                    fat32::lookup(subpath, &mut cluster, &mut sz)?;
                    Ok((cluster, sz))
                });
                match r {
                    Ok(v) => v,
                    Err(e) => {
                        fd_clear(idx);
                        return Err(e);
                    }
                }
            }
            Fs::None => {
                fd_clear(idx);
                return Err(Errno::Inval);
            }
        };

        fd_set(idx, ino, size, fs, 0, slot as u8);
        let fd = fd_get(idx);
        Ok(fd_token(idx, fd.epoch))
    }
}

/// # Safety
///
/// Caller contract: token must come from the fd table of the calling
/// context; close only touches this context's own fd table.
pub unsafe fn close(token: FdToken) -> KResult<()> {
    // SAFETY: fd_check validates idx and epoch; fd_clear only marks the
    // owning context's slot unused.
    unsafe {
        let idx = fd_check(token)?;
        // Master-close frees the PTY pair (slave fds then see EPIPE).
        let f = fd_get(idx);
        if f.fs == Fs::Devfs
            && let Some(pair) = devfs::ptym_ino_idx(f.ino)
        {
            crate::fs::pty::free(pair);
        }
        fd_clear(idx);
        Ok(())
    }
}
