use crate::drivers::{
    entropy, gpio, hwrand, i2c, led, rtc, sdhci, spi, usb, virtio, virtio_console, virtio_input,
...
pub(crate) unsafe fn probe_peripherals() {
    unsafe {
        ...
        let probe_at = |b: usize| {
            ...
            if virtio_input::probe(b) && virtio_input::init(b).is_ok() {
        ...
        // Seed the xoshiro PRNG from the active hardware source now that all
        // entropy drivers have been probed, before init/login can run.
        entropy::seed();
        crate::kinf!(
            "entropy",
            "prng seeded from %s",
