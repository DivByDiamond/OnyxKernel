...
pub unsafe fn init(dram_base: u64, dram_size: u64) {
    ...
    unsafe {
        ...
        *p = Pmm {
        ...
        // Kernel image, BSS, heap reserve and the bitmap itself all lie
        // below `data_base`, which is the physical address of bitmap bit 0.
        // The managed range `[base, base + total_pages*PAGE_SIZE)` therefore
        // never covers reserved memory, so no explicit reserved-marking pass
        // is needed — those pages are unaddressable by the allocator.
        crate::srv::klog::emit(
