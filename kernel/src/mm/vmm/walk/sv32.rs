...
pub unsafe fn walk(root_pa: u64, vaddr: u64, leaf_level: u32, create: bool) -> KResult<*mut u64> {
    // SAFETY: every volatile read/write targets `table_pa + idx*4` with
    // idx < PTES_PER_TABLE, where `table_pa` starts at the (valid) root
    // and each step comes from a valid non-leaf PTE or a freshly
    // zero-allocated table from `pmm::alloc_zero`. Mutations require the
    // VMM lock per contract.
    unsafe { walk_impl(root_pa, vaddr, leaf_level, create) }
}
...
unsafe fn walk_impl(root_pa: u64, vaddr: u64, leaf_level: u32, create: bool) -> KResult<*mut u64> {
    unsafe {
        ...
        for level in (leaf_level + 1..=1).rev() {
            let idx = match level {
                1 => bits::l1_idx(vaddr as bits::Va),
        ...
        let idx = match leaf_level {
            0 => bits::l0_idx(vaddr as bits::Va),
            1 => bits::l1_idx(vaddr as bits::Va),
            _ => return Err(Errno::Inval),
        };
...
unsafe fn split_leaf(parent_pte_ptr: *mut u64, parent_pte: u64, parent_level: u32) -> KResult<()> {
    // SAFETY: VMM lock held per contract; all volatile writes target the
    // freshly zero-allocated table or the caller-provided live PTE slot.
    unsafe {
        let new_pa = pmm::alloc_zero()?;
