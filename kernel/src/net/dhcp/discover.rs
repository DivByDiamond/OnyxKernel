use crate::drivers::{hwrand, virtio_net};
use crate::net::poll;
use crate::net::udp;
...
pub unsafe fn dhcp_discover() -> KResult<([u8; 4], [u8; 4], [u8; 4], [u8; 4])> {
    unsafe {
        ...
        // any L2 host race us with forged OFFER/ACK frames.
        let xid = hwrand::next_u32();
        ...
        for _ in 0..50000 {
            ...
            if let Ok(n) = udp::udp_recv(sock, &mut buf)
                && let Some((msg_type, yiaddr, sid, mask, dns)) =
                    protocol::parse_dhcp_reply(&buf[..n], xid, &mac)
                && msg_type == MSG_OFFER
                && !yiaddr.iter().all(|&b| b == 0)
            {
                offered_ip = yiaddr;
                server_id = sid;
                subnet_mask = mask;
        ...
        let request =
            protocol::make_dhcp_msg(MSG_REQUEST, xid, &mac, Some(offered_ip), Some(server_id));
        ...
        for _ in 0..50000 {
            ...
            let mut buf = [0u8; 2048];
            if let Ok(n) = udp::udp_recv(sock, &mut buf)
                && let Some((msg_type, yiaddr, _sid, mask, dns)) =
                    protocol::parse_dhcp_reply(&buf[..n], xid, &mac)
                && msg_type == MSG_ACK
            {
