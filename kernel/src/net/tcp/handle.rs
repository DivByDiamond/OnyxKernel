...
pub unsafe fn tcp_connect(dst_ip: [u8; 4], port: u16) -> KResult<usize> { unsafe {
...
pub unsafe fn tcp_send(cid: usize, data: &[u8]) -> KResult<usize> { unsafe {
...
pub unsafe fn tcp_recv(cid: usize, buf: &mut [u8]) -> KResult<usize> { unsafe {
    let conn = CONNS[cid].as_mut().ok_or(Errno::Inval)?;
    if conn.recv_len == 0 {
...
pub unsafe fn tcp_close(cid: usize) { unsafe {
...
pub unsafe fn handle_tcp(frame: &[u8], ip_start: usize, ihl: usize, total_len: usize) { unsafe {
